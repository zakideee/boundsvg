...
fn exhaustive_largest_fitting_size(font_context: &FontContext<'_>) -> f64 {
    ...
    for probe_index in 0..REQUIRED_PROBES {
        ...
        let result = layout_text(&candidate_request, font_context).expect("exact candidate layout");
        let fits = result.overflow.overflow_type == "none"
            && result.lines.len() <= 1
            && result.bbox.w <= candidate_request.max_width + 0.001
            && result.bbox.h <= candidate_request.max_height.unwrap() + 0.001;
        if fits {
...
fn negative_tracking_forces_exact_grid_even_with_a_certified_flow_provider() {
    ...
    let result = layout_flow_with_regions(&flow_request(&nodes), &font_context, &regions)
        .expect("content-aware exact-grid flow fit");

    assert_eq!(result.chosen_font_size_px, Some(41.5));
...
fn grow_requires_the_authored_size_to_fit_before_considering_a_larger_island() {
    ...
    let result = layout_text(&grow_request, &font_context).expect("ineligible grow layout");
    ...
    assert_eq!(result.chosen_font_size_px, MIN_SIZE_PX);

...
| Document-synchronized SVG timeline        | Supported     | `playback: { mode: "timeline", durationMs, iterations }` compiles representable tracks onto one document clock. Spring and other non-exact mappings fail; live discontinuities use the documented two-sided carve-out. |

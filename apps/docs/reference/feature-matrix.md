...
| Declarative animation (`animate`)         | Supported     | Opacity and center-pivot transform with cubic, step, and spring easing. `renderToAnimatedSvg` preserves authored tracks independently; static APIs sample `timeMs`. Viewer playback timing is outside the contract. |
| Document-synchronized SVG timeline        | Not supported | 0.3 supports only `playback: { mode: "independent" }`; no document duration, phase synchronization, or document-level iterations.                                                                                   |
...
| Independent animated SVG output                   | Supported    | Explicit `renderToAnimatedSvg(..., { playback: { mode: "independent" } })`; no document timeline.                     |

...
use crate::parser::{LazyArray16, LazyArray32, Offset, Offset16, Offset32, Stream, F2DOT14};
...
// PATCHED (boundsvg vendored fix, see README-PATCH.md): a well-formed
// 65535-glyph font has 65536 glyph-variation-data offsets, so the offset
// count and index are widened to `u32` — the same treatment upstream applied
// to `loca` in commit 3a193ba. Still unfixed in upstream `gvar` as of
// 2026-08-20.
#[derive(Clone, Copy)]
enum GlyphVariationDataOffsets<'a> {
    Short(LazyArray32<'a, Offset16>),
    Long(LazyArray32<'a, Offset32>),
}
...
impl<'a> Table<'a> {
    ...
    pub fn parse(data: &'a [u8]) -> Option<Self> {
        ...
        let offsets = {
            // The number of offsets is `glyphCount + 1`, which for a
            // 65535-glyph font is 65536 and does not fit in `u16`.
            let offsets_count = u32::from(glyph_count) + 1;
            let is_long_format = flags & 1 == 1; // The first bit indicates a long format.
            if is_long_format {
                GlyphVariationDataOffsets::Long(s.read_array32::<Offset32>(offsets_count)?)
            } else {
                GlyphVariationDataOffsets::Short(s.read_array32::<Offset16>(offsets_count)?)
            }
        };
    ...
    #[cfg(test)]
    fn offsets_len(&self) -> u32 {
        match self.offsets {
            GlyphVariationDataOffsets::Short(offsets) => offsets.len(),
            GlyphVariationDataOffsets::Long(offsets) => offsets.len(),
        }
    }
    ...
    fn parse_variation_data(
        ...
        let (start, end) = match self.offsets {
            GlyphVariationDataOffsets::Short(ref array) => {
                ...
                (
                    array.get(u32::from(glyph_id.0))?.to_usize() * 2,
                    array.get(u32::from(next_glyph_id))?.to_usize() * 2,
                )
            }
            GlyphVariationDataOffsets::Long(ref array) => (
                array.get(u32::from(glyph_id.0))?.to_usize(),
                array.get(u32::from(next_glyph_id))?.to_usize(),
            ),
        };
...
mod boundsvg_patch_tests {
    ...
    fn keeps_every_offset_for_the_maximum_glyph_count() {
        ...
        let variation_data_offset = HEADER_LEN + offsets_count * 4;
        let mut data = vec![0u8; variation_data_offset];
        ...
        data[16..20].copy_from_slice(&(variation_data_offset as u32).to_be_bytes());

        let table =
            Table::parse(&data).expect("the maximum glyph-count gvar table must remain available");

        // Every offset survives, including the one for the last valid glyph
        // id (65534) — a u16 offset count could hold at most 65535 of them.
        assert_eq!(table.offsets_len(), u32::from(u16::MAX) + 1);
    }
}
